...
test('YouTube import validates input, converts real audio and preserves job lifecycle',async t=>{
  ...
  await t.test('browser submits conversion, refreshes library, plays FLAC and cancels jobs',async()=>{
    ...
    try{
      ...
      const job=page.locator('.youtube-job').filter({hasText:'Âm thanh thử UIVIDEO0001'});await job.locator('[data-play]').waitFor();const downloaded = page.waitForEvent('download'); await job.locator('[data-download]').click(); const file = await downloaded; assert.match(file.suggestedFilename(), /\.flac$/); assert.equal(await file.failure(), null);
